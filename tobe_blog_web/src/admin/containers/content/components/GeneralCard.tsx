...
export function GeneralCard(props: {record: GeneralCardData, onClick?: (id: string | number) => void, operations: Operation[]}) {
  ...
  return (<Grid item xs={12} sm={6}key={props.record.id}>
            <Card variant='outlined' sx={{borderRadius: 4}}>
              <Grid container>
                <Grid item xs={12} sm={12} md={8} lg={8} xl={8} sx={{p: 3, flexDirection: 'column', height: '260px'}}>
                    <Grid container>
                      <Grid flexGrow={1}>
                      ...
                      <Grid flexGrow={0}>
                        <Typography color='textSecondary' variant='body2'>{dateMonFormat(props.record.createTime)}</Typography>
                      </Grid>
                    </Grid>
                    <Typography
                    ...
                    <Grid container sx={{my: 1}}>
                      {props.record?.tags?.map((t) => <Chip key={`${props.record.id}-tag-${t.label}`} label={t.label} variant='outlined' sx={{borderRadius: 3, height: "28px", p: 0, mr: 1, fontWeight: 400, color: theme.palette.text.secondary}}/>)}
                    </Grid>
                    <Grid container>
                      ...
                      <Grid item alignItems='center' sx={{display: 'inline-flex'}} >
                          <VisibilityIcon color='disabled' sx={{width: '16px', mr: 1}}/> 
                          <Typography color='textSecondary' variant='body2' sx={{mr: 1}}>{props.record.viewCount}</Typography>
                          <FavoriteIcon color='disabled' sx={{width: '16px', mr: 1}}/> 
                          <Typography color='textSecondary' variant='body2'>{props.record.likeCount}</Typography>
                      </Grid>
                    </Grid>
                </Grid>
